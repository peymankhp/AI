    ...
    try:
        ...
        if st.button("Train Model"):
            with st.spinner("Training in progress..."):
                ...
                # Calculate metrics (account for sequence length offset)
                train_metrics = DataProcessor.calculate_metrics(
                    y_train[params['seq_length']:],
                    train_pred[params['seq_length']:]
                )
                test_metrics = DataProcessor.calculate_metrics(
                    y_test[params['seq_length']:],
                    test_pred[params['seq_length']:]
                )
                ...
                train_weekly = DataProcessor.resample_weekly(
                    pd.DataFrame({
                        'actual': y_train[params['seq_length']:],
                        'predicted': train_pred[params['seq_length']:]
                ...
                test_weekly = DataProcessor.resample_weekly(
                    pd.DataFrame({
                        ...
                        'predicted': test_pred[params['seq_length']:]
