...
from tensorflow.keras.models import Sequential, Model  # Neural network container
from tensorflow.keras.layers import LSTM, Dense, Dropout, Input, Concatenate, BatchNormalization, LeakyReLU  # Neural network layers
from tensorflow.keras.losses import Huber  # Huber loss function
from tensorflow.keras.optimizers import Adam  # Adam optimizer

def create_lstm_model(input_shape):
    """
    Build sophisticated hybrid neural network for temporal pattern recognition
    Args:
    ...
    """
    try:
        logging.info(f"Creating enhanced hybrid model with input shape: {input_shape}")
        
        # Input layer
        input_layer = Input(shape=input_shape, name="Input")
        
        # Path 1: Deep LSTM for long-term patterns
        lstm1_1 = LSTM(units=256, return_sequences=True, name="LSTM1_1")(input_layer)
        batch_norm1_1 = BatchNormalization()(lstm1_1)
        dropout1_1 = Dropout(0.3)(batch_norm1_1)
        
        lstm1_2 = LSTM(units=128, return_sequences=True, name="LSTM1_2")(dropout1_1)
        batch_norm1_2 = BatchNormalization()(lstm1_2)
        dropout1_2 = Dropout(0.3)(batch_norm1_2)
        
        lstm1_3 = LSTM(units=64, return_sequences=False, name="LSTM1_3")(dropout1_2)
        batch_norm1_3 = BatchNormalization()(lstm1_3)
        dropout1_3 = Dropout(0.3)(batch_norm1_3)
        
        # Path 2: Shallow but wider LSTM for short-term patterns
        lstm2_1 = LSTM(units=384, return_sequences=False, name="LSTM2_1")(input_layer)
        batch_norm2_1 = BatchNormalization()(lstm2_1)
        dropout2_1 = Dropout(0.3)(batch_norm2_1)
        
        # Path 3: Bidirectional LSTM for pattern detection in both directions
        from tensorflow.keras.layers import Bidirectional
        bilstm = Bidirectional(LSTM(units=192, return_sequences=False, name="BiLSTM"))(input_layer)
        batch_norm3 = BatchNormalization()(bilstm)
        dropout3 = Dropout(0.3)(batch_norm3)
        
        # Merge all paths
        merged = Concatenate()([dropout1_3, dropout2_1, dropout3])
        
        # Dense layers for feature combination with advanced activation
        dense1 = Dense(256)(merged)
        batch_norm4 = BatchNormalization()(dense1)
        leaky1 = LeakyReLU(alpha=0.1)(batch_norm4)
        dropout4 = Dropout(0.2)(leaky1)
        
        dense2 = Dense(128)(dropout4)
        batch_norm5 = BatchNormalization()(dense2)
        leaky2 = LeakyReLU(alpha=0.1)(batch_norm5)
        dropout5 = Dropout(0.2)(leaky2)
        
        dense3 = Dense(64)(dropout5)
        batch_norm6 = BatchNormalization()(dense3)
        leaky3 = LeakyReLU(alpha=0.1)(batch_norm6)
        dropout6 = Dropout(0.2)(leaky3)
        
        # Output layer
        output_layer = Dense(1, name="Output")(dropout6)
        
        # Create model
        model = Model(inputs=input_layer, outputs=output_layer, name="EnhancedGoldPriceLSTM")
        ...
        model.compile(
            optimizer=optimizer,
            loss=Huber(delta=1.0),  # Robust loss function
            metrics=['mae', 'mse']
        )
        
        logging.info("Enhanced hybrid model created successfully")
        model.summary(print_fn=logging.info)
        return model
        
    except Exception as e:
        logging.error(f"Error creating enhanced hybrid model: {str(e)}")
        raise
...
def train_time_series_model(model, X_train, y_train, validation_data, epochs=150):
    """
    Enhanced training process with advanced callbacks
    Args:
    ...
    model_path = os.path.join(MODEL_DIR, f'enhanced_gold_price_model_{timestamp}.h5')
    
    # Configure enhanced callbacks
    callbacks = [
        EarlyStopping(
            monitor='val_loss',
        ...
        ReduceLROnPlateau(
            ...
            min_lr=1e-6,
            verbose=1,
            cooldown=5  # Wait before resuming normal operation
        )
    ]
    
    try:
        logging.info("Starting enhanced model training...")
        # Execute model training with adjusted parameters
        history = model.fit(
            X_train, y_train,
            validation_data=validation_data,
            epochs=epochs,
            batch_size=32,  # Kept the same as it worked well
            callbacks=callbacks,
            verbose=1,
            shuffle=True  # Enable shuffling for better generalization
        )
        
        logging.info(f"Enhanced model training completed. Best model saved to {model_path}")
        return history
        
    except Exception as e:
        logging.error(f"Error during enhanced model training: {str(e)}")
        raise
